...
/* ============================================================================
 * API: 预览 sitemap URL 目录
 * Route: POST /core/dataset/collection/create/urlDirectory/preview
 * ============================================================================ */
export const PreviewUrlDirectoryBodySchema = z.object({
  datasetId: z.string().meta({ description: '数据集 ID' }),
  sitemapUrl: z.string().url().meta({ description: 'URL directory entry URL' }),
  limit: z.number().int().min(0).max(1000).default(0).meta({ description: '最大返回数量' })
});
export type PreviewUrlDirectoryBodyType = z.infer<typeof PreviewUrlDirectoryBodySchema>;

export const PreviewUrlDirectoryResponseSchema = z.object({
  total: z.number().int().min(0).meta({ description: '解析出的 URL 总数' }),
  urls: z.array(z.string().url()).meta({ description: 'URL 列表' })
});
export type PreviewUrlDirectoryResponseType = z.infer<
  typeof PreviewUrlDirectoryResponseSchema
>;
