...
import {
  isMcpServerEnabled,
  isTemplateMarketEnabled
} from '@/web/common/system/featureFlags';
import { normalizeWorkflowTemplateConfig } from '@/pageComponents/dashboard/agent/utils/appTemplateParse';
...
const CreateAppsPage = () => {
  ...
  const showMcpTools = isMcpServerEnabled(feConfigs);
  const showTemplateMarket = isTemplateMarketEnabled(feConfigs);
  ...
  const { data: templateData, loading: isLoadingTemplates } = useRequest(
    ...
    {
      manual: false,
      ready: showTemplateMarket,
      refreshDeps: [selectedAppType, showTemplateMarket]
    }
  );
  ...
  useEffect(() => {
    if (!showMcpTools && selectedAppType === AppTypeEnum.mcpToolSet) {
      setSelectedAppType(AppTypeEnum.httpToolSet);
      setValue('avatar', createAppTypeMap[AppTypeEnum.httpToolSet]?.icon || '');
    }
  }, [selectedAppType, showMcpTools, setValue]);
  ...
  const { runAsync: onClickCreate, loading: isCreating } = useRequest(
    async (
      ...
      if (templateId) {
        ...
        const normalizedWorkflow = normalizeWorkflowTemplateConfig(templateDetail.workflow);
    ...
    <Box h={'100vh'} overflow={'hidden'}>
      ...
      <Flex bg={'white'} flex={1} gap={7} p={6} h={'calc(100vh - 60px)'}>
        ...
        >
          <Box mb={5} borderBottom={'1px solid'} borderColor={'myGray.200'}>
            ...
            <SimpleGrid columns={3} gap={2.5} pb={5}>
              {Object.values(createAppTypeMap)
                ...
                .filter((option) => showMcpTools || option.type !== AppTypeEnum.mcpToolSet)
                .map((option) => (
                  <AppTypeCard
                    key={option.type}
          ...
          {showTemplateMarket && templateData?.list && templateData.list.length > 0 && (
            <Box>
