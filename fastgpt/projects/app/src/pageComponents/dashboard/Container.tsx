...
const DashboardContainer = ({
  ...
  useEffect(() => {
    if (
      ...
      (!hasAppCreatePer || !isTemplateMarketEnabled(feConfigs))
    ) {
      router.replace('/dashboard/agent');
    }
  }, [currentTab, feConfigs, hasAppCreatePer, router, userInfo]);

  // Template market
  const { data: templateTags = [], loading: isLoadingTemplatesTags } = useRequest(
    () =>
      currentTab === TabEnum.app_templates && hasAppCreatePer && isTemplateMarketEnabled(feConfigs)
    ...
    {
      manual: false,
      refreshDeps: [currentTab, feConfigs, hasAppCreatePer, userInfo?.team.isWecomTeam]
    }
  );
  const { data: templateData, loading: isLoadingTemplates } = useRequest(
    ...
    {
      manual: false,
      refreshDeps: [currentTab, appType, feConfigs, hasAppCreatePer]
    }
  );
